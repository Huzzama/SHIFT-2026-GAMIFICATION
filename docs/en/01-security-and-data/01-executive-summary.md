...
- **The mentor** is local by default: it answers on the device, with no AI model. If the institution turns it on (`VITE_MENTOR_MODE=gemini` and `GEMINI_API_KEY` on the backend), open conversation goes to Gemini **through the backend, never directly from the browser**, with an eleven-field context (none of which identifies the person), the student's message and the last 8 turns of the conversation. If Gemini does not answer, the local mentor does.
...
| Component | State |
|---|---|
| Backend with token, allow list, restricted CORS, rate limit, no token leakage in logs | **Implemented and tested** (36 automated checks in total) |
