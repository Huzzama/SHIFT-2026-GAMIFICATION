...
| T26 | Otra extensión instalada en el mismo navegador llama al backend local (en *loopback* se acepta cualquier origen `chrome-extension://`) | Datos, clave de Gemini | Solo cuando el servidor escucha en *loopback*; se apaga con `FARO_STRICT_ORIGINS=true` y sola en cualquier otra interfaz; el tope por cliente del mentor sigue aplicando; en despliegue, id exacto de la extensión + `FARO_STRICT_ORIGINS=true` | [código] / Política | "an extension origin is refused unless extension origins are enabled", "config: half a Canvas setup is refused, mentor-only is accepted" |
