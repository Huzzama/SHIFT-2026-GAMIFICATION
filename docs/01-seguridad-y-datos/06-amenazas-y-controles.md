...
| T14 | El servicio de IA recibe datos personales | Identidad, confianza | Contexto de 11 campos construido en una sola función; sin nombre, correo, ids, calificaciones | [código] | — |
...
| T18 | El backend arranca sin credencial y sirve datos vacíos como si fueran reales | Confianza | Se niega a arrancar sin `CANVAS_ACCESS_TOKEN`; con token inválido responde `canvas_token_rejected` | [código] | "a wrong token is reported as rejected" |
...
| T20 | Robo del archivo `server/.env` | Token | Alcance limitado a lectura de un estudiante; revocación de un clic en Canvas; el camino OAuth2 elimina el archivo | Política / [pendiente] | — |
