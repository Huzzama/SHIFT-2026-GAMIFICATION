...
| **El mentor de IA ve once campos y ningún dato personal.** | El mentor es local por omisión. Si se activa Gemini, recibe, a través del backend, el contexto de once campos, el mensaje y los últimos 8 turnos. El contexto se construye en un solo archivo (`src/lib/mentorContext.ts`) que no tiene acceso al nombre, correo, foto ni calificaciones, y el backend lo vuelve a filtrar (`server/src/mentor.ts`). | [código] [prueba] |
