...
- [ ] ⚙ `npm test` en `server/` pasa las 20 verificaciones.
- [ ] ⚙ `curl http://127.0.0.1:3000/health` responde con el host de Canvas y **no** contiene el token.
...
- [ ] ⚖ Existe un aviso de privacidad (simplificado + completo) revisado por jurídico, que menciona: las seis rutas de Canvas, los datos que FARO crea, el estado "pasó algo en mi vida", el futuro mentor remoto y la retención.
- [ ] ⚖ Cada participante firmó el consentimiento del piloto (capítulo 7.4).
...
- [ ] Mentor remoto tras el backend, con contrato de datos que prohíba entrenamiento.
